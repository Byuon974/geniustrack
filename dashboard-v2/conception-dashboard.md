...
## 5. Plan de mise en œuvre (après validation de la maquette)

1. Compléter le `ProjetRepository` avec une méthode de demandes en attente par valideur, et exposer les agrégations manquantes (créneaux du jour, projets encadrés).
2. Adapter le `DashboardController` pour préparer un jeu de données différent selon le rôle de l'utilisateur connecté.
3. Décliner le template en deux partiels (vue admin, vue formateur) partageant les mêmes composants de carte, pour ne pas dupliquer la présentation (DRY).
4. Vérifier les accès : chaque vue ne doit exposer que les données du périmètre du rôle.

> Réserve : la maquette fixe l'agencement et la logique, pas le pixel final. Le rendu réel dépendra de l'intégration dans les composants Twig existants et de la charte du projet. Les chiffres de la maquette sont illustratifs.
